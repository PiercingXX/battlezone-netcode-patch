...
> If your extracted folder name differs, replace `Battlezone Netcode Patch` in all commands below.
...
3. Start buffer logging: `& "$HOME\Downloads\Battlezone Netcode Patch\buffer-logging\buffer_logger_windows.ps1" -Action Start`
...
7. Stop buffer logging: `& "$HOME\Downloads\Battlezone Netcode Patch\buffer-logging\buffer_logger_windows.ps1" -Action Stop`
8. Send the generated `.zip` bundle from `test_bundles`.

Linux (all Steam variants):

1. Start buffer logging:
    `cd "$HOME/Downloads/Battlezone Netcode Patch" && ./buffer-logging/buffer_logger_linux.sh start "/home/$USER/.local/share/Steam/steamapps/common/Battlezone 98 Redux"`
...
5. Stop buffer logging:
    `cd "$HOME/Downloads/Battlezone Netcode Patch" && ./buffer-logging/buffer_logger_linux.sh stop`
...
```powershell
cd "$HOME\Downloads\Battlezone Netcode Patch"
...
```powershell
& "$HOME\Downloads\Battlezone Netcode Patch\buffer-logging\buffer_logger_windows.ps1" -Action Start
```
...
```powershell
& "$HOME\Downloads\Battlezone Netcode Patch\buffer-logging\buffer_logger_windows.ps1" -Action Stop
```
